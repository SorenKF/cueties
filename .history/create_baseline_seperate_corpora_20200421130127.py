...
def add_baseline_info(corpus_test_folder, cues, baseline_path):
    """
    Combines the information gathered in add_baseline_column for  to a single dataframe,
    ...
    """
    # Add baseline column and write it.
    df_input = add_baseline_column(corpus_test_folder, cues)

    # Add individual dataframes together and write to file.
    complete_df = input_folder.append(corpus_test_folder, ignore_index=True)
    df_input.to_csv(baseline_path, index=False, index_label=False)
