...
def import_attribution_doc(filename):
    ...
    :returns df: a pandas dataframe with columns corresponding to those in the file:
            "filename", "sentence_number", "doc_token_number", "sent_token_number", "offset", "word", "lemma", "POS",
            "dependency_label", "dependency_head_id", "attribution"
    """
...
def extract_attributions(attribution_df):
    ...
    :returns attributions: a list of dictionaries of span tuples. An attribution consists of a source,
        a cue, and a content, and each of these has an associated span (start and end indices). The
    ...
    for i in range(num_attributions):
        attributions.append({"SOURCE": None, "CUE": None, "CONTENT": None})
