...
def get_fp_dicts(filepath):
    ...
    bigrams_following_fp = []
    bigrams_preceding_fp = []
    ...
    for index, prediction in enumerate(df['predicted_cue_label']):
        if df.iloc[index]['cue_label'] == 0 and df.iloc[index]['predicted_cue_label'] == 1:
            ...
            bigrams_following_fp.append(df['bigram_following_lemma'].iloc[index])
            bigrams_preceding_fp.append(df['bigram_prev_lemma'].iloc[index])
    ...
    bigrams_following_fp_dict = Counter(bigrams_following_fp)
    bigrams_preceding_fp_dict = Counter(bigrams_preceding_fp)
    
    return(pos_fp_dict, lemma_fp_dict, dependency_fp_dict, bigrams_preceding_fp_dict, bigrams_following_fp_dict)
...
def get_fn_dicts(filepath):   
    ...
    bigrams_following_fn = []
    bigrams_preceding_fn = []
    ...
    for index, prediction in enumerate(df['predicted_cue_label']):
        if df.iloc[index]['cue_label'] == 1 and df.iloc[index]['predicted_cue_label'] == 0:
            ...
            bigrams_following_fn.append(df['bigram_following_lemma'].iloc[index])
            bigrams_preceding_fn.append(df['bigram_prev_lemma'].iloc[index])
    ...
    bigrams_following_fn_dict = Counter(bigrams_following_fn)
    bigrams_preceding_fn_dict = Counter(bigrams_preceding_fn)
    
    return(pos_fn_dict, lemma_fn_dict, dependency_fn_dict, bigrams_preceding_fn_dict, bigrams_following_fn_dict)
...
def get_tp_dicts(filepath):
    ...
    for index, prediction in enumerate(df['predicted_cue_label']):
        if df.iloc[index]['cue_label'] == 1 and df.iloc[index]['predicted_cue_label'] == 1:
            ...
            bigrams_following_c.append(df['bigram_following_lemma'].iloc[index])
            bigrams_preceding_c.append(df['bigram_prev_lemma'].iloc[index])
            
    # dicts
    pos_correct_dict = Counter(pos_correct)
    lemma_correct_dict = Counter(lemma_correct)
    dependency_correct_dict = Counter(dependency_labels_correct) 
    bigrams_following_c_dict = Counter(bigrams_following_c)
    bigrams_preceding_c_dict = Counter(bigrams_preceding_c)
    
    return (pos_correct_dict, lemma_correct_dict, dependency_correct_dict, bigrams_preceding_c_dict, bigrams_following_c_dict)
